...
#
#		( This is a modified version of the original makefile - loyc12 )
#
...
WIN_COMP_PATH		?= C:/raylib/w64devkit/bin
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
	...
	ifeq ($(OS),Windows_NT)
		PLATFORM_OS=WINDOWS
		export PATH := $(WIN_COMP_PATH):$(PATH)
	else
		UNAMEOS=$(shell uname)
		ifeq ($(UNAMEOS),Linux)
			PLATFORM_OS=LINUX
		else ifeq ($(UNAMEOS),FreeBSD)
			PLATFORM_OS=BSD
		else ifeq ($(UNAMEOS),OpenBSD)
			PLATFORM_OS=BSD
		else ifeq ($(UNAMEOS),NetBSD)
			PLATFORM_OS=BSD
		else ifeq ($(UNAMEOS),DragonFly)
			PLATFORM_OS=BSD
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
	...
	ifeq ($(PLATFORM_OS),WINDOWS)
		...
		else ifeq ($(RAYLIB_LIBTYPE),SHARED)
